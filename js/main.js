...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("player", "assets/garrettM24.png");
  ...
  create() {
    // ... (seu código de criação já existente)

    // adiciona um ouvinte para detectar mudanças de orientação
    this.scale.on("orientationchange", (orientation) => {
      if (orientation === Phaser.Scale.PORTRAIT) {
        console.log("O dispositivo está em modo retrato");
        // ajuste os elementos para o modo retrato se necessário
      } else {
        console.log("O dispositivo está em modo paisagem");
        // ajuste os elementos para o modo paisagem se necessário
      }
    });

    // exemplo de verificação do dispositivo
    ...
    this.garrettM24 = this.physics.add.sprite(gameWidth / 2, 100, "player");
    this.garrettM24.setBounce(0.35); //define uma rebatida leve ao colidir
    this.garrettM24.setCollideWorldBounds(true); //impede que o turbo saia dos limites do jogo
    this.garrettM24.setDepth(1); //define o depth do turbo para que fique na frente do fogo
    ...
    platforms.forEach((pos) => {
      ...
      this.physics.add.collider(this.garrettM24, bumper); //adiciona a colisão entre o personagem e a plataforma
    ...
    this.pirelliTyres = this.physics.add.staticImage(
      gameWidth / 1.25,
      gameHeight / 1.25,
      "pirelliTyres"
    );
    ...
    this.gasCan = this.physics.add.staticImage(
      gameWidth / 6,
      gameHeight / 1.25,
      "gasCan"
    );
    ...
    this.placar = this.add
      .text(
        gameWidth / 2,
        50,
        "Money to pay the mechanic for his work: " + this.pontuacao,
        {
          fontSize: "25px",
          fill: "#ffffff",
          fontFamily: "Arial",
  ...
  update() {
    ...
    if (this.teclado.left.isDown) {
      this.garrettM24.setVelocityX(-250); //se a seta esquerda estiver pressionada, move para a esquerda
    } else if (this.teclado.right.isDown) {
      this.garrettM24.setVelocityX(250); //se a seta direita estiver pressionada, move para a direita
    } else {
      this.garrettM24.setVelocityX(0); //caso contrário, para o movimento horizontal
    }
    ...
    if (this.teclado.up.isDown) {
      this.garrettM24.setVelocityY(-375); //define a velocidade vertical para cima (pulo)
    ...
    this.fire.setPosition(
      this.garrettM24.x + this.garrettM24.width / 3,
      this.garrettM24.y + this.garrettM24.height / 2
    );

    //se o personagem cair abaixo da tela (acima de 1080 + 50 pixels), inicia a cena de game over
    if (this.garrettM24.y > 1080 + 50) {
