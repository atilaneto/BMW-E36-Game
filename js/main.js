...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("background", "assets/e36shop.jpg");      //carrega a imagem de fundo do jogo
    this.load.image("player", "assets/garretM24.png");          //carrega a imagem do personagem
    this.load.image("fire", "assets/fire.png");                 //carrega a imagem do efeito de fogo (turbo)
    this.load.image("frontBumper", "assets/frontBumperM3.png");   //carrega a imagem da plataforma/barreira
    this.load.image("money", "assets/money.png");               //carrega a imagem do money (para pontuação)
  ...
  create() {
    ...
    this.fire.setScale(0.125);                 //reduz a escala do fogo para 12.5% (50% menor que 25%)
    ...
    //cria o personagem (jogador)
    this.garretM24 = this.physics.add.sprite(gameWidth / 2, 100, "player");
    this.garretM24.setBounce(0.2);              //define uma rebatida leve ao colidir
    this.garretM24.setCollideWorldBounds(true); //impede que o personagem saia dos limites do jogo
    this.garretM24.setDepth(1);                //define o depth do personagem para que fique na frente do fogo

    //define os controles do jogo utilizando as setas do teclado
    this.teclado = this.input.keyboard.createCursorKeys();

    //cria as plataformas (barreiras) usando um array e um loop
    this.frontBumpers = [];
    const platforms = [
    ...
    //adiciona elementos extras ao cenário (pneus e galão de gasolina)
    ...
    //cria o money (para pontuação) e configura suas propriedades
    ...
    //define a sobreposição (overlap) entre o personagem e o money para coletar o money
  ...
  update() {
    ...
    //controle de pulo/ativação do turbo:
    //se a seta para cima estiver pressionada, o personagem pula e o efeito de fogo é ativado
    if (this.teclado.up.isDown) {
      this.garretM24.setVelocityY(-150);   //define a velocidade vertical para cima (pulo)
  ...
  coletarMoney() {
    ...
    //se a pontuação atingir 50 ou mais pontos, transita para a cena de game over
...
//configuração do phaser com o canvas centralizado na tela
const config = {
